      ...
      <div>
        <div class="overflow-scroll">
          <div class="row"
            ...
            <div class="col-5 border mx-2" v-for="list in lists" :key="list"
              style="display: inline-block;vertical-align: top;" @click="goList(list.id)">
              <div class="row mt-2">
                <img :src="list.coverImgUrl" class="img-fluid" alt="">
                <h5>{{list.name}}</h5>
              </div>
              <div class="row ">
                <p class="small text-muted d-flex align-items-center p1"
                  style="height: 150px; word-break: break-all; white-space: normal;">{{list.description}}</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getListSongs1: function (id) {
      var that = this
      axios.get('http://localhost:3001/playlist/detail?id=' + id).then(function (response) {
